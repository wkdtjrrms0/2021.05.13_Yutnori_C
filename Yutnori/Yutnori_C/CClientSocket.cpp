...
void CClientSocket::OnReceive(int nErrorCode)
{
	...
	default:
		...
		if (sort == 0) { //채팅
			...
			if (strcmp(receivenick, pMain->m_nickname) ==0) { //받은 닉네임, 클라이언트닉네임이 같은경우
				if (count != 1) {
					count = 1;
					pMain->IsTurn = -1;
			...
			else if (strcmp(receivenick, pMain->m_nickname) != 0) { //받은 닉네임, 클라이언트닉네임이 다른경우
				count = -1;
			...
			else if (strcmp(receivenick, pMain->m_nickname) != 0) { //받은 닉네임, 클라이언트닉네임이 다른경우=>나의 패배
			...
			if (strcmp(receivenick, pMain->m_nickname) == 0) { //받은 닉네임, 클라이언트닉네임이 같은경우=>나의 패배
				pMain->m_ctrlEdit.ReplaceSel(_T("게임에 패배하셨습니다.\r\n"));
				pMain->InitPiece();
			}
...
CString CClientSocket::Find(LPCTSTR receive)
{
	...
	if (find.Left(6) == _T("[Chat]")) { //메시지가 채팅일 경우
		...
		if (strcmp(find.Mid(find.Find(": Win") + 2,3), _T("Win")) == 0) { //승리메시지=>[Game]CBNU JSK: Win\r\n => 반환=>CBNU JSKWin
			sort = 3;
